...
# BrainFlow is imported lazily so the web app can launch in simulation mode even
# when hardware-specific dependencies are missing or incompatible.
def ensure_pkg_resources_compat():
    """Provide the tiny pkg_resources API BrainFlow still uses.

    Recent setuptools versions removed pkg_resources. Current BrainFlow still uses
    pkg_resources.resource_filename(...) to locate its native libraries, so we
    provide a small compatibility shim instead of pinning setuptools to an old
    version.
    """
    try:
    ...
    def resource_filename(package_or_requirement, resource_name):
        spec = importlib.util.find_spec(str(package_or_requirement))
        if spec is None or spec.origin is None:
            raise ModuleNotFoundError(f"Cannot locate package {package_or_requirement!r}")
...
def get_brainflow_api():
    ensure_pkg_resources_compat()
    try:
        from brainflow.board_shim import BoardShim, BrainFlowInputParams, BoardIds, BrainFlowError
    except Exception as exc:
        raise RuntimeError(
            "BrainFlow hardware support is not available in this environment. "
            "Install/update BrainFlow, or use Simulation Mode. Original error: "
            f"{exc}"
        ) from exc
    return BoardShim, BrainFlowInputParams, BoardIds, BrainFlowError


def create_brainflow_params():
    _, BrainFlowInputParams, _, _ = get_brainflow_api()
    params = BrainFlowInputParams()
    params.serial_port = os.environ.get("PIEEG_SERIAL_PORT", "/dev/spidev0.0")
    return params
...
# EMG recording buffer — filled only while recording_active is True.
# Mirrors recording_data / recording_sampling_rate for the secondary EMG
# stream so both modalities can be exported to BIDS at stop-recording time.
emg_recording_data = []
emg_recording_sampling_rate = 250
...
def process_and_emit_chunk(data_transposed, sampling_rate, acquisition_ts=None, signal_type=None):
    ...
    try:
        ...
        # Both modalities get their own recording buffers so stop-recording can
        # export a BIDS-compliant EDF for each signal independently.
        if sig == 'emg' and dual_stream_mode:
            update_emg_collected_buffer(data_for_frontend, effective_rate)
            buffer_emg_recording_data(data_for_frontend, effective_rate)
        else:
...
def buffer_emg_recording_data(chunk, sampling_rate):
    """Append secondary EMG stream samples to emg_recording_data while recording.

    This is the dual-stream counterpart of buffer_recording_data.  It is only
    called from process_and_emit() when sig == 'emg' and dual_stream_mode is
    True, ensuring the EEG and EMG recording buffers are always independent and
    can be exported as two separate BIDS-compliant modality folders.
    """
    global emg_recording_data, emg_recording_sampling_rate
    if not recording_active or recording_paused:
        return
    if not chunk:
        return
    with recording_lock:
        if not emg_recording_data or len(emg_recording_data) != len(chunk):
            emg_recording_data = [[] for _ in range(len(chunk))]
        for idx, channel_data in enumerate(chunk):
            emg_recording_data[idx].extend(channel_data)
            max_samples = int(sampling_rate * max_record_seconds)
            if len(emg_recording_data[idx]) > max_samples:
                emg_recording_data[idx] = emg_recording_data[idx][-max_samples:]
        emg_recording_sampling_rate = sampling_rate
...
def reset_recording_state():
    """Reset recording-related state (EEG and EMG)"""
...
def _load_simulation_from_fif(path):
    ...
    # Smart unit conversion: MNE normally returns data in volts (SI), but some
    # FIF files written by OpenBCI or non-standard exporters already store values
    # in µV (the calibration metadata is set incorrectly so MNE does not scale).
    # Heuristic: genuine volt-scale EEG has |values| < 10 mV (0.01 V); µV-scale
    # data has |values| in the tens-to-thousands range.  We convert only when
    # the data looks like it really is in volts.
    typical_abs = float(np.percentile(np.abs(data), 95)) if data.size else 0.0
    if typical_abs < 0.01:          # data is in volts → convert to µV
        data = data * 1e6
        logging.info("FIF loader: data appears to be in volts → converted to µV (×1e6)")
    else:
        logging.info("FIF loader: data already in µV range (typical |val|=%.1f) → no conversion", typical_abs)
...
def _load_simulation_from_edf(path):
    ...
    data = raw.get_data(picks=picks).astype(np.float32, copy=False)
    # Same smart unit heuristic as the FIF loader — some EDF files are already in µV.
    typical_abs = float(np.percentile(np.abs(data), 95)) if data.size else 0.0
    if typical_abs < 0.01:
        data = data * 1e6
        logging.info("EDF loader: data appears to be in volts → converted to µV (×1e6)")
    else:
        logging.info("EDF loader: data already in µV range (typical |val|=%.1f) → no conversion", typical_abs)
    channel_names = [raw.ch_names[idx] for idx in picks]
    channel_types = [raw.get_channel_types(picks=[idx])[0] for idx in picks]
...
def read_eeg_data_file_simulation():
    ...
    try:
        ...
        # Notify the frontend of the file length, channel count, and channel
        # names so the stacked labels show e.g. "FP1" instead of "Channel 1".
        channel_names = (
            simulation_loaded_data.get('metadata', {}).get('channel_names') or
...
def read_eeg_data_brainflow():
    ...
    board = None
    try:
        BoardShim, _, BoardIds, _ = get_brainflow_api()
        params = create_brainflow_params()
        ...
        if e.__class__.__name__ == "BrainFlowError":
            logging.error(f"BrainFlow error: {str(e)}")
            socketio.emit('error', {'message': f"BrainFlow error: {str(e)}"})
        else:
            logging.error(f"Unexpected BrainFlow hardware error: {e}")
            socketio.emit('error', {'message': f"Unexpected BrainFlow hardware error: {str(e)}"})
        running = False
    finally:
        try:
...
def start_recording():
    ...
    with recording_lock, event_lock:
        ...
        recording_paused = False
        recording_data = []
        emg_recording_data = []          # reset EMG buffer for fresh recording
        event_markers = []
        ...
        recording_sampling_rate = current_stream_sampling_rate
        emg_recording_sampling_rate = current_stream_sampling_rate
...
def stop_recording():
    ...
    In single-stream mode the ZIP contains the usual EEG BIDS files
    (sub-.../eeg/*.edf + *.json + *_channels.tsv + *_events.tsv).

    In dual-stream mode the ZIP contains *two* complete BIDS modality
    folders side-by-side:
        sub-XX/ses-YY/eeg/   ← primary EEG stream
        sub-XX/ses-YY/emg/   ← secondary EMG stream
    Both share the same markers/events so stimuli can be time-locked
    across modalities in downstream analysis.
    """
    if not recording_active:
        return jsonify({"status": "error", "message": "Recording is not active"}), 400
    ...
    # Snapshot both buffers under the lock so no new data can be appended
    # while we are packaging the ZIP.
    with recording_lock:
        data_copy     = [list(ch) for ch in recording_data]     if recording_data     else []
        emg_data_copy = [list(ch) for ch in emg_recording_data] if emg_recording_data else []
        emg_rate      = float(emg_recording_sampling_rate or effective_sampling_rate)
    with event_lock:
    ...
    try:
        if not data_copy or not any(len(ch) for ch in data_copy):
            ...
            return jsonify({"status": "error", "message": "No recorded EEG data available"}), 400

        # ── EEG BIDS export ──────────────────────────────────────────────────
        metadata_fields = load_metadata_fields(
            subject_id, session_id, task, run, modality,
            effective_sampling_rate, len(data_copy)
        )
        persist_metadata_fields(metadata_fields, subject_id, session_id, task, run, modality)

        eeg_file_paths = save_bids_recording(
            data_copy, markers_copy, subject_id, session_id, task, run,
            effective_sampling_rate, modality, metadata_fields
        )

        # ── EMG BIDS export (dual-stream only) ───────────────────────────────
        emg_file_paths = {}
        has_emg = dual_stream_mode and emg_data_copy and any(len(ch) for ch in emg_data_copy)
        if has_emg:
            emg_metadata_fields = load_metadata_fields(
                subject_id, session_id, task, run, 'emg',
                emg_rate, len(emg_data_copy)
            )
            persist_metadata_fields(emg_metadata_fields, subject_id, session_id, task, run, 'emg')
            emg_file_paths = save_bids_recording(
                emg_data_copy, markers_copy, subject_id, session_id, task, run,
                emg_rate, 'emg', emg_metadata_fields
            )
            logging.info(
                f"Dual-stream EMG export: {len(emg_data_copy)} ch × "
                f"{len(emg_data_copy[0]) if emg_data_copy else 0} samples @ {emg_rate} Hz"
            )
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            # EEG files
            for label, path in eeg_file_paths.items():
                if path and os.path.exists(path):
                    zip_file.write(path, arcname=os.path.relpath(path, BIDS_ROOT))
            # EMG files (only present in dual-stream mode)
            for label, path in emg_file_paths.items():
                if path and os.path.exists(path):
                    zip_file.write(path, arcname=os.path.relpath(path, BIDS_ROOT))

            # Top-level metadata summary
            eeg_duration = len(data_copy[0]) / effective_sampling_rate if data_copy and data_copy[0] else 0
            emg_duration = len(emg_data_copy[0]) / emg_rate if has_emg and emg_data_copy[0] else 0
            zip_file.writestr('recording_metadata.json', json.dumps({
                ...
                "eeg": {
                    ...
                    "n_channels": len(data_copy)
                },
                "emg": {
                    "sampling_rate": emg_rate,
                    "duration_seconds": emg_duration,
                    "n_channels": len(emg_data_copy)
                } if has_emg else None,
        ...
        zip_buffer.seek(0)
        reset_recording_state()
        stream_label = "dual" if has_emg else modality
        logging.info(
            f"Recording packaged ({stream_label}, subject={subject_id}, task={task}, run={run})"
        )
        return send_file(
            zip_buffer,
            mimetype='application/zip',
            as_attachment=True,
            download_name=f"sub-{subject_id}_ses-{session_id}_task-{task}_run-{run}_{stream_label}_recording.zip"
        )
    except Exception as e:
